...
interface DeleteMockConfirmDialogProps {
  open: boolean;
  mockName: string;
  onConfirm: () => void;
  onCancel: () => void;
...
export default function DeleteMockConfirmDialog({
  open,
  mockName,
  onConfirm,
  onCancel,
  isPending,
}: DeleteMockConfirmDialogProps) {
  const [inputValue, setInputValue] = useState("");
  const isConfirmed = inputValue === mockName;
      ...
      <DialogContent>
        <DialogHeader>
          <div className="flex items-center gap-2 text-destructive">
            ...
            <DialogTitle>Delete Mock</DialogTitle>
          </div>
          <DialogDescription>
            This action cannot be undone. This will permanently delete this mock
            endpoint and any applications using it will stop working.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded border border-destructive/20 bg-gradient-to-t from-destructive/10 to-card px-4 py-3">
          ...
          >
            Type <strong className="normal-case text-destructive">"{mockName}"</strong> to confirm:
          </label>
          <input
